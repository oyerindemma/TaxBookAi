...
export async function PATCH(req: Request, context: RouteContext) {
  ...
  try {
    ...
    if (action !== "save" && action !== "approve" && action !== "reject") {
    ...
    if (draft.ledgerTransaction?.id) {
      return NextResponse.json(
        {
          error:
            "This draft has already been posted into the ledger and can no longer be updated from receipt review.",
        },
        { status: 409 }
      );
    }
    ...
    const proposedDate =
      normalizeDateInput(body.transactionDate) ?? draft.proposedDate ?? new Date();
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const updatedLearningPayload =
        action === "reject"
          ...
          : buildReceiptLearningFeedback({
              ...
              action: action === "approve" ? "approve" : "save",
              vendorName,
      ...
      await tx.bookkeepingDraft.update({
        ...
        data: {
          ...
          reviewedAt:
            action === "save" ? draft.reviewedAt : reviewTimestamp,
          approvedAt:
            action === "approve"
              ? reviewTimestamp
              : action === "reject"
    ...
    logRouteError("receipt draft review failed", error, {
      workspaceId: ctx.workspaceId,
      userId: ctx.userId,
      draftId,
    });
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Unable to review receipt draft",
      },
      { status: 500 }
    );
  }
