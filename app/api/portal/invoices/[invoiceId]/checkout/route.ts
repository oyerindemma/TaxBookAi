...
export async function POST(req: Request, context: RouteContext) {
  ...
  try {
    ...
    if (!invoice.client?.email) {
      return NextResponse.json(
        { error: "This invoice is missing a billing email for checkout." },
        { status: 400 }
      );
    }
    ...
    const callbackUrl = `${getAppUrl()}/portal/invoices/${invoice.id}?provider=paystack`;
    ...
    logRouteError("portal invoice checkout initialization failed", error, {
