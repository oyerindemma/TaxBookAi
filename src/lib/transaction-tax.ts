...
function buildStoredTaxValues(input: {
  ...
  const vatRate =
    input.vatTreatment !== "NONE" && input.vatTreatment !== "EXEMPT"
      ? normalizeRate(input.vatRate) || NIGERIA_TAX_CONFIG.vat.standardRate
      : 0;
  const whtRate =
    input.whtTreatment !== "NONE"
      ? normalizeRate(input.whtRate) || NIGERIA_TAX_CONFIG.wht.heuristicDefaultRate
      : 0;
  ...
  return {
    ...
    vatRate:
      input.suggestedVatTreatment !== "NONE" && input.suggestedVatTreatment !== "EXEMPT"
        ? suggestedSignals.vatRate || NIGERIA_TAX_CONFIG.vat.standardRate
        : 0,
    whtRate:
      input.suggestedWhtTreatment !== "NONE"
        ? suggestedSignals.whtRate || NIGERIA_TAX_CONFIG.wht.heuristicDefaultRate
        : 0,
    vatAmountMinor:
      input.suggestedVatTreatment === "NONE" || input.suggestedVatTreatment === "EXEMPT"
        ? 0
...
async function loadTransactionTaxOptions(
  ...
  try {
    const [clientBusinesses, bankAccounts, categories] = await Promise.all([
...
export async function getWorkspaceTransactionTaxSummary(input: TransactionTaxFilters) {
  ...
  try {
    ...
    const [records, previousRecords, options] = await Promise.all([
      loadTransactionTaxRecords(where, useCompatibleQuery),
      previousWhere
        ? loadTransactionTaxRecords(previousWhere, useCompatibleQuery)
        : Promise.resolve([] as TransactionTaxRecord[]),
      loadTransactionTaxOptions(input.workspaceId),
    ]);
