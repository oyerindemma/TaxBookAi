...
function clearOfflineStorage() {
  try {
    ...
    localStorage.removeItem(OFFLINE_WORKSPACE_KEY);
  } catch {
    return;
  }
}
      ...
      try {
        ...
        const nextWorkspaceId =
          typeof payload.activeWorkspaceId === "number"
            ? String(payload.activeWorkspaceId)
            : null;
        const previousUserId = localStorage.getItem(OFFLINE_SESSION_USER_KEY);
        const previousWorkspaceId = localStorage.getItem(OFFLINE_WORKSPACE_KEY);
        ...
        if (nextWorkspaceId) {
          localStorage.setItem(OFFLINE_WORKSPACE_KEY, nextWorkspaceId);
        } else {
          localStorage.removeItem(OFFLINE_WORKSPACE_KEY);
        }
      } catch {
