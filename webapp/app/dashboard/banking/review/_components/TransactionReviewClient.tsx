...
import LedgerReportReadinessWidget from "@/app/dashboard/_components/LedgerReportReadinessWidget";
...
type AccountingPostingStatus = "UNPOSTED" | "READY_TO_POST" | "POSTED";
type WorkflowStatus =
  | "empty"
  | "NOT_STARTED"
  | "IN_REVIEW"
...
function formatAccountingPostingStatus(value: AccountingPostingStatus) {
  if (value === "READY_TO_POST") return "Ready to post";
  if (value === "POSTED") return "Posted";
  return "Unposted";
}

function accountingPostingStatusClassName(value: AccountingPostingStatus) {
  if (value === "POSTED") {
    return "border-emerald-200 bg-emerald-50 text-emerald-900";
  }
  if (value === "READY_TO_POST") {
...
function canPostTransaction(transaction: ReviewTransaction) {
  return transaction.accountingPostingStatus === "READY_TO_POST";
}
...
type SinglePostingResponse =
  ...
  | {
      ...
      code: PostingFailureCode;
      reason: string;
      transactionId: number;
    };

type BulkPostingResponse = {
  ok: true;
  results: SinglePostingResponse[];
...
export default function TransactionReviewClient({
  ...
  const selectedPostableIds = selectedTransactions
    .filter((transaction) => canPostTransaction(transaction))
    .map((transaction) => transaction.id);
  const selectedNotReadyCount = selectedTransactions.length - selectedPostableIds.length;
  ...
  async function handlePostTransaction(transactionId: number) {
    if (!editable || postingId !== null) return;
    ...
    try {
      const response = await fetch("/api/banking/transactions/posting", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      const payload = await parseJson<SinglePostingResponse & { error?: string }>(response);

      if (!response.ok || !payload) {
        throw new Error(payload?.error ?? "Failed to post the transaction.");
      }
      ...
      setMessage("Transaction posted to the ledger. Reports have been refreshed.");
      await loadReviewQueue(filters);
      setSelectedTransactionId(transactionId);
      setSelectedIds((current) => {
      ...
      setError(postingError instanceof Error ? postingError.message : "Network error.");
    } finally {
      setPostingId(null);
    }
  }
  ...
  async function handleBulkPost() {
    if (!editable || bulkPosting || selectedPostableIds.length === 0) return;
    ...
    try {
      const response = await fetch("/api/banking/transactions/posting/bulk", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          transactionIds: selectedPostableIds,
        }),
      });
      ...
      if (!response.ok || !payload) {
        setFieldErrors(payload?.fieldErrors ?? {});
        throw new Error(payload?.error ?? "Failed to post selected transactions.");
      }

      const failedIds = new Set(
        payload.results.filter((result) => !result.ok).map((result) => result.transactionId)
      );
      selectedTransactions
        .filter((transaction) => !canPostTransaction(transaction))
        .forEach((transaction) => failedIds.add(transaction.id));

      setSelectedIds(failedIds);
      if (payload.postedCount > 0) {
        const detailParts = [
          `${payload.postedCount} posted`,
          payload.reviewNeededCount > 0 ? `${payload.reviewNeededCount} still need review` : null,
          payload.skippedCount > 0 ? `${payload.skippedCount} skipped` : null,
          selectedNotReadyCount > 0 ? `${selectedNotReadyCount} not ready` : null,
        ].filter(Boolean);

        setMessage(
          `${detailParts.join(" · ")}. Reports have been refreshed for the active workspace.`
        );
      } else {
        const firstFailure = payload.results.find((result) => !result.ok);
        throw new Error(firstFailure?.reason ?? "No selected transactions could be posted.");
      }
  ...
  return (
    <section className="space-y-6">
      ...
      <LedgerReportReadinessWidget summary={data.workflow} />
          ...
          <CardContent className="flex flex-wrap items-center justify-between gap-3 pt-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Apply a workspace-scoped review update, generate bookkeeping suggestions, or post
                ready items in one pass.
              </p>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                disabled={bulkPosting || selectedPostableIds.length === 0}
              >
                {bulkPosting
                  ? "Posting..."
                  : selectedPostableIds.length > 0
                    ? `Post ready (${selectedPostableIds.length})`
                    : "No ready items"}
              </Button>
              <Button
                variant="outline"
                onClick={() => setSelectedIds(new Set())}
                disabled={bulkUpdating || bulkApproving || bulkPosting}
              >
                Clear selection
              </Button>
            </div>
                    ...
                    <tr key={transaction.id} className="align-top hover:bg-muted/30">
                      ...
                      <td className="border-b px-3 py-3">
                        <div className="space-y-2">
                          ...
                          <Badge
                            variant="outline"
                            className={accountingPostingStatusClassName(
                              transaction.accountingPostingStatus
                            )}
                          >
                            {formatAccountingPostingStatus(transaction.accountingPostingStatus)}
                          </Badge>
                      ...
                      <td className="border-b px-3 py-3">
                        <div className="flex flex-wrap gap-2">
                          {editable && canPostTransaction(transaction) ? (
                            <Button
                              size="sm"
                              onClick={() => handlePostTransaction(transaction.id)}
                              disabled={postingId === transaction.id}
                            >
                              {postingId === transaction.id ? "Posting..." : "Post to ledger"}
                            </Button>
                          ) : null}
                          <TransactionReviewRowActions
                            ...
                            canEdit={editable}
                            canPost={canPostTransaction(transaction)}
                            onOpen={() => setSelectedTransactionId(transaction.id)}
                            onPost={() => handlePostTransaction(transaction.id)}
                            onMarkReviewed={() =>
            ...
            <div className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span>
                        {formatAccountingPostingStatus(
                          selectedTransaction.accountingPostingStatus
                        )}
                      </span>
                    </div>
                    <div className="flex items-center justify-between gap-3">
              ...
              <form className="space-y-4" onSubmit={handleSaveSelectedTransaction}>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {editable && canPostTransaction(selectedTransaction) ? (
                    <Button
                      type="button"
                      disabled={postingId === selectedTransaction.id}
                      onClick={() => handlePostTransaction(selectedTransaction.id)}
                    >
                      {postingId === selectedTransaction.id
                        ? "Posting..."
                        : "Post to ledger"}
                    </Button>
                  ) : null}
