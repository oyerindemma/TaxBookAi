...
function transactionTypeLabel(type: "CREDIT" | "DEBIT") {
...
function buildTransactionForm(transaction: Transaction): TransactionForm {
  ...
  return {
    ...
    splitLines: [
      {
        clientId: `transaction-${transaction.id}-split-line-1`,
        description: transaction.description,
        reference: transaction.reference ?? "",
      ...
      {
        clientId: `transaction-${transaction.id}-split-line-2`,
        description: "",
        reference: "",
        amount: "0.00",
        vendorName: "",
...
export default function ReconcileClient({
  ...
  function addSplitLine(transaction: Transaction, form: TransactionForm) {
    ...
    updateForm(transaction.id, "splitLines", [
      ...form.splitLines,
      {
        clientId: createClientKey(`transaction-${transaction.id}-split-line`),
        description: "",
        reference: "",
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">
            Import bank CSVs, review each line, and post clean transactions to the ledger.
          </p>
        </div>
      ...
      {!dashboard?.aiConfigured && (
        <Card className="border-dashed">
          <CardContent className="pt-6 text-sm text-muted-foreground">
            AI suggestions are off. Import still works with simple rules; review each suggestion
            before posting.
          </CardContent>
        </Card>
      )}
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Choose the business, pick the bank account, preview the CSV, then import rows for
              review. Sample file:{" "}
          ...
          {noBusinesses ? (
            <div className="rounded-lg border border-dashed px-4 py-4 text-sm text-muted-foreground">
              Add a business profile before importing a statement.
            </div>
          ) : null}
          {noAccounts ? (
            <div className="rounded-lg border border-dashed px-4 py-4 text-sm text-muted-foreground">
              Add the bank account for this statement, then preview the CSV.
            </div>
          ) : null}
                              ...
                              <div className="mt-3 grid gap-3">
                                ...
                                >
                                  ...
                                  {preview.headers.map((header, headerIndex) => (
                                    <option key={`${field.key}-${header}-${headerIndex}`} value={header}>
                                      {header}
                          ...
                          <table className="min-w-full text-left text-xs">
                            ...
                            <tbody>
                              {preview.previewRows.map((row, rowIndex) => (
                                <tr
                                  key={`${row.transactionDate ?? row.date ?? ""}-${row.amount ?? ""}-${row.reference ?? ""}-${rowIndex}`}
                                  className="border-t"
                                >
            ...
            <div className="space-y-2">
              {accounts.length === 0 ? (
                <div className="rounded-md border border-dashed px-3 py-4 text-sm text-muted-foreground">
                  No bank accounts yet. Add the account that appears on the statement.
                </div>
              ) : (
      ...
      <Card>
        ...
        <CardContent>
          {imports.length === 0 ? (
            <div className="space-y-3 rounded-md border border-dashed px-4 py-5 text-sm text-muted-foreground">
              <p>No statements imported yet.</p>
              <p>Choose a business, select a bank account, upload a CSV, then preview it.</p>
            </div>
          ) : (
            <div className="space-y-3">
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="space-y-3 pt-6 text-sm text-muted-foreground">
              <div>
                {noAccounts
                  ? "Create a bank account, then import a CSV statement."
                  : imports.length === 0
                    ? "Import a statement to create review items."
                    : queueHasFilters
                      ? "No transactions match these filters. Clear filters or import another statement."
                      : "The review queue is clear. Import another statement when you have new activity."}
              </div>
              {queueHasFilters ? (
                <Button
                  type="button"
          ...
          displayTransactions.map((transaction) => {
            ...
            const manualLedgerLabel =
              transaction.type === "DEBIT" ? "Record Money Out" : "Record Money In";
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-sm text-muted-foreground">
                        ...
                        {transaction.balanceAmountMinor !== null
                          ? ` · Running Balance ${formatMoney(transaction.balanceAmountMinor, transaction.currency)}`
                          : ""}
                      </div>
                    </div>
                  </div>
