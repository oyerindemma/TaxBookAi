...
export default function ComplianceToolsCard({
  ...
  const [pendingAction, setPendingAction] = useState<
    "archive" | "delete-account" | "export-account" | "export-workspace" | null
  >(null);
  ...
  async function waitForExportJob(jobId: string) {
    for (let attempt = 0; attempt < 80; attempt += 1) {
      const response = await fetch(`/api/compliance/export/jobs/${jobId}`, {
        cache: "no-store",
      });
      ...
      if (!response.ok) {
        throw new Error(data?.error ?? "Unable to check export status right now.");
      }
      ...
      if (data?.status === "failed") {
        throw new Error("Export failed while processing. Please try again.");
      }
  ...
  async function exportData(scope: "account" | "workspace") {
    const pendingKey = scope === "account" ? "export-account" : "export-workspace";
    setPendingAction(pendingKey);
    setActionError(null);
    setActionMessage("Preparing your export in the background.");

    try {
      const response = await fetch(`/api/compliance/export?scope=${scope}`, {
        method: "POST",
      });
      const data = (await response.json().catch(() => null)) as
        | {
      ...
      if (!response.ok || !data?.jobId) {
        ...
        setActionError(data?.error ?? "Unable to queue the export right now.");
        return;
      }

      setActionMessage("Export queued. We will download it as soon as it is ready.");
      const downloadUrl = await waitForExportJob(data.jobId);
      setActionMessage("Export ready. Your download is starting.");
      window.location.assign(downloadUrl);
    } catch (error) {
      setActionMessage(null);
      setActionError(error instanceof Error ? error.message : "Unable to export data right now.");
    } finally {
          ...
          <div className="rounded-2xl border bg-background p-4">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <Button
                variant="outline"
                size="sm"
                disabled={pendingAction === "export-account" || pendingAction === "export-workspace"}
                onClick={() => exportData("account")}
              >
                {pendingAction === "export-account" ? (
              ...
              {canExportWorkspace ? (
                <Button
                  size="sm"
                  disabled={
                    pendingAction === "export-account" || pendingAction === "export-workspace"
                  }
                  onClick={() => exportData("workspace")}
                >
                  {pendingAction === "export-workspace" ? (
                    <Loader2 className="size-4 animate-spin" />
                  ) : (
                    <Download className="size-4" />
                  )}
