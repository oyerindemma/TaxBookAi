...
export default async function IntegrityPage() {
  ...
  const [initialIntegrityResult, initialHealthSnapshot] = await Promise.all([
    getFinancialIntegrityIssuesSnapshot({
      ...
      .catch((error) => {
        ...
        return {
          snapshot: buildFinancialIntegrityIssuesFallbackSnapshot({
            workspaceIds: initialWorkspaceId ? [initialWorkspaceId] : accessibleWorkspaceIds,
            selectedWorkspaceId: initialWorkspaceId,
          }),
          errorMessage: "Unable to load integrity issues right now.",
        };
      }),
  ...
  const initialSnapshot = initialIntegrityResult.snapshot;
  const initialError = initialIntegrityResult.errorMessage;
