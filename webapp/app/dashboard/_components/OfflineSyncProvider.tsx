...
function createId() {
  ...
  if (typeof crypto !== "undefined" && typeof crypto.getRandomValues === "function") {
    ...
    return `offline-${Array.from(bytes, (byte) => byte.toString(16).padStart(2, "0")).join("")}`;
  }

  offlineIdCounter += 1;
  return `offline-${offlineIdCounter}`;
}
...
function buildWorkspaceMismatchConflict(
  action: OfflineQueuedAction,
  ...
  return {
    ...
    action,
    message:
      "This queued change belongs to a different workspace. Switch back to that workspace before retrying, or dismiss it.",
    current: {
      label: action.target.label,
      status: `Active workspace: ${activeWorkspaceId ?? "none"}`,
      lastKnownChangeAt: null,
      href: action.target.href,
      recordType: action.target.recordType,
...
export function OfflineSyncProvider({
  ...
  async function syncNow() {
    ...
    try {
      while (queuedActionsRef.current.length > 0 && navigator.onLine) {
        ...
        try {
          if (!isOfflineQueuedActionForWorkspace(currentAction, activeWorkspaceId)) {
            setQueuedActions((current) =>
              current.filter((action) => action.id !== currentAction.id)
            );
            setConflicts((current) => [
              ...current,
              buildWorkspaceMismatchConflict(currentAction, activeWorkspaceId),
            ]);
            continue;
          }
  ...
  async function submitAction<TPayload>(
    ...
    if (!isOfflineQueuedActionForWorkspace(action, activeWorkspaceId)) {
      const conflict = buildWorkspaceMismatchConflict(action, activeWorkspaceId);
  ...
  async function retryConflict(conflictId: string) {
    ...
    if (!isOfflineQueuedActionForWorkspace(nextAction, activeWorkspaceId)) {
      setConflicts((current) => [
        ...current,
        buildWorkspaceMismatchConflict(nextAction, activeWorkspaceId),
      ]);
      return;
    }
