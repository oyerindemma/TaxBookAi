...
import { recheckFinancialIntegrityIssueById } from "@/lib/financial-integrity";
...
export async function POST(_req: Request, context: RouteContext) {
  ...
  try {
    const result = await recheckFinancialIntegrityIssueById({
      issueId,
    ...
    logRouteError("financial integrity recheck failed", error, {
      issueId,
      workspaceId: issue.workspaceId,
      userId: auth.context.userId,
    });
    return NextResponse.json(
      { error: "Unable to recheck this integrity issue right now." },
      { status: 500 }
    );
  }
}
