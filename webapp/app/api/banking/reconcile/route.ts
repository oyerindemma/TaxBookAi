...
import { resolveWorkspaceIdOrDefault } from "@/lib/workspace-fallback";
import {
...
export async function GET(req: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      buildEmptyBankingDashboard({
        status: "error",
        error: "Failed to load banking dashboard",
      }),
      { status: 200 }
    );
  }
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (workspaceId !== ctx.workspaceId) {
      return NextResponse.json(
        { error: "workspaceId does not match the active workspace" },
        { status: 403 }
      );
    }
    ...
    console.error("[TaxBook:reconcile]", {
      workspaceId,
