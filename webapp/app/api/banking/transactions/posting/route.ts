...
import {
  BankTransactionPostingNotFoundError,
  postWorkspaceBankTransaction,
} from "@/lib/bank-transaction-posting";
import { validateBankTransactionPostingPayload } from "@/lib/bank-transaction-posting-validation";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const validation = validateBankTransactionPostingPayload(body);
    ...
    logRouteError("bank transaction posting failed", error, {
      workspaceId: access.ctx.workspaceId,
    ...
    const message =
      error instanceof Error ? error.message : "Failed to post the bank transaction.";
