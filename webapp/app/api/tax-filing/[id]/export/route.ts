...
function buildScheduleCsv(detail: NonNullable<Awaited<ReturnType<typeof getTaxFilingDetail>>>) {
  const metadataRows = [
    ["Report", `${detail.draft.taxType} filing schedule`],
    ["Workspace", detail.workspace.name],
    ["Business", detail.clientBusiness.name ?? detail.workspace.businessName ?? detail.workspace.name],
    ["Period", detail.period.label],
    ["DraftStatus", detail.draft.status],
    ["GeneratedAt", new Date().toISOString()],
    ["Basis", "Prepared by TaxBook AI for accountant review and manual filing support"],
    [],
  ];
  ...
  if (detail.draft.taxType === "VAT") {
    return toCsvRows([
      ...
      ...detail.scheduleRows.map((row) => [
        ...
        ((Number(row.basisAmountMinor ?? 0)) / 100).toFixed(2),
        Number(row.vatAmountMinor ?? 0),
        ((Number(row.vatAmountMinor ?? 0)) / 100).toFixed(2),
        String(row.currency ?? ""),
  ...
  if (detail.draft.taxType === "WHT") {
    return toCsvRows([
      ...
      ...detail.scheduleRows.map((row) => [
        ...
        ((Number(row.whtAmountMinor ?? 0)) / 100).toFixed(2),
        String(row.currency ?? ""),
        Array.isArray(row.flags) ? row.flags.join(" | ") : "",
      ]),
    ]);
  ...
  return toCsvRows([
    ...metadataRows,
    ["Label", "Direction", "AmountMinor", "Amount", "TaxAmountMinor", "TaxAmount", "Status", "Flags"],
    ...detail.scheduleRows.map((row) => [
      String(row.label ?? ""),
      String(row.direction ?? ""),
      ...
      ((Number(row.taxAmountMinor ?? 0)) / 100).toFixed(2),
      String(row.status ?? ""),
      Array.isArray(row.flags) ? row.flags.join(" | ") : "",
    ]),
  ]);
...
function buildChecklistCsv(detail: NonNullable<Awaited<ReturnType<typeof getTaxFilingDetail>>>) {
  return toCsvRows([
    ["Report", `${detail.draft.taxType} filing checklist`],
    ["Workspace", detail.workspace.name],
    ["Business", detail.clientBusiness.name ?? detail.workspace.businessName ?? detail.workspace.name],
    ["Period", detail.period.label],
    ["DraftStatus", detail.draft.status],
