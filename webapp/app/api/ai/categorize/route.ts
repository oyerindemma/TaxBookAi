...
import { autoCategorizeWorkspaceTransactions } from "@/lib/ai/autoCategorize";
...
function validateBatchPayload(payload: BatchPayload) {
  if (payload.transactionIds.length > 200) {
    return "Auto-categorization is limited to 200 transactions per batch.";
  }
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const result = await autoCategorizeWorkspaceTransactions({
    ...
    logRouteError("AI auto-categorization failed", error, {
      workspaceId: access.ctx.workspaceId,
