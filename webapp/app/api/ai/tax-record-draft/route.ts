...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!hasOpenAiServerConfig()) {
      const suggestion = addAiSafetyToSuggestion(buildFallbackTextSuggestion(text));
    ...
    const prompt =
      ...
      "Your output is a draft for accountant review, not legal or tax advice. " +
      ...
      "Do not state that a record is filing-ready or legally compliant. " +
    ...
    if (!response.ok) {
      ...
      const metadata = buildAiSuggestionMetadata({
        ...
        sourceType: "text",
        warnings: [
          data?.error?.message ?? "OpenAI request failed. Using local bookkeeping heuristics.",
        ],
      });
    ...
    const suggestion = addAiSafetyToSuggestion(normalizeBookkeepingSuggestion(draft));
    const legacyDraft = buildLegacyTaxRecordDraft(suggestion);
    const metadata = buildAiSuggestionMetadata({
      provider: "openai",
      route: "tax-record-draft" as const,
      model,
      sourceType: "text" as const,
      warnings: buildAiSafetyWarnings({
