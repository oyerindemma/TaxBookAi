...
export async function PATCH(req: Request, context: RouteContext) {
  ...
  try {
    ...
    const adjustmentId = parseId(id);
    if (!adjustmentId) {
      return NextResponse.json({ error: "Invalid CIT adjustment id." }, { status: 400 });
    }
    ...
    const detail = await updateCitAdjustment({
    ...
    logRouteError("cit adjustment update failed", error, {
      workspaceId: ctx.workspaceId,
      userId: ctx.userId,
    });

    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to update the CIT adjustment." },
      { status: 400 }
    );
  }
}
...
export async function DELETE(_req: Request, context: RouteContext) {
  ...
  try {
    ...
    const detail = await deleteCitAdjustment({
      workspaceId: ctx.workspaceId,
      adjustmentId,
      actorUserId: ctx.userId,
    });
    ...
    logRouteError("cit adjustment delete failed", error, {
      workspaceId: ctx.workspaceId,
      userId: ctx.userId,
    });

    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to delete the CIT adjustment." },
      { status: 400 }
    );
  }
}
