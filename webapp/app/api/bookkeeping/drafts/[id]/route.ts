...
import { postLedgerTransactionToFinancialEngineWithExecutor } from "@/lib/accounting/postTransaction";
...
export async function PATCH(req: Request, context: RouteContext) {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (action === "approve") {
        const postedAmountMinor = totalAmountMinor ?? amountMinor ?? 0;
        ...
        if (transaction.entryId) {
          await postLedgerTransactionToFinancialEngineWithExecutor(tx, {
            id: transaction.entryId,
            workspaceId: ctx.workspaceId,
            amount: postedAmountMinor,
            direction,
            transactionDate: proposedDate,
            description,
            reference: reference || null,
