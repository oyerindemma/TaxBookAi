...
export async function getWorkspaceShellState(userId: number) {
  let activeMembership: ActiveWorkspaceMembership | null = null;
  let workspaceSummaries: UserWorkspaceSummary[] = [];

  try {
    activeMembership = await getActiveWorkspaceMembership(userId);
  } catch (error) {
    console.error(
  ...
  try {
    workspaceSummaries = await listUserWorkspaceSummaries(userId);
  } catch (error) {
    console.error(
