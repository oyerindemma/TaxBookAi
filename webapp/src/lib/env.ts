...
import { isStubPaymentModeAllowed } from "@/lib/paystack-security";
...
export function getPaystackServerConfig() {
  ...
  const webhookSecret =
    getDeploymentStage() === "production"
      ...
      : readEnv("PAYSTACK_WEBHOOK_SECRET") || secretKey;
...
export function getHealthCheckSecret() {
  return readEnv("HEALTH_CHECK_SECRET") || null;
}
...
export function getPaymentRuntimeConfig() {
  ...
  return {
    ...
    allowStubPayments: isStubPaymentModeAllowed({
      deploymentStage,
      explicitAllowStubPayments: explicitStubSetting,
    }),
  };
}
...
export function getEnvironmentHealth(
  ...
  if (deploymentStage === "production" && !readEnv("HEALTH_CHECK_SECRET")) {
    warnings.push("HEALTH_CHECK_SECRET not set; /api/health requires an admin session instead of a monitor token");
  }
  if (!hasAnyEnv(["REDIS_URL", "BULLMQ_REDIS_URL", "UPSTASH_REDIS_URL"])) {
    ...
    if (deploymentStage === "production") {
      ...
      warnings.push("REDIS_URL not set; async compliance exports will be unavailable");
    }
  }
  ...
  return {
    ...
    hasRedisUrl: hasAnyEnv(["REDIS_URL", "BULLMQ_REDIS_URL", "UPSTASH_REDIS_URL"]),
