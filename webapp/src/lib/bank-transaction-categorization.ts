...
function getExpectedCategoryTypes(transaction: CategorizationTransaction): LedgerCategoryType[] {
  ...
  return transaction.type === "CREDIT"
    ? ["INCOME", "LIABILITY", "EQUITY", "ASSET"]
    : ["EXPENSE", "ASSET", "LIABILITY", "OTHER", "EQUITY"];
}
...
function buildHintCategories(transaction: CategorizationTransaction) {
  ...
  if (transaction.suggestedType === "TRANSFER") {
    ...
    addHint("Cash and bank", "the narration suggests a cash movement between accounts");
  }
  if (transaction.suggestedType === "OWNER_DRAW") {
    addHint("Owner drawings", "the narration looks like an owner draw");
  }
  if (/(capital|owner funding|shareholder|equity injection)/.test(narration)) {
    addHint("Owner capital", "capital contribution language appears in the narration");
  }
  if (/(loan|facility|borrowing|credit line)/.test(narration)) {
    addHint("Loans and borrowings", "loan or borrowing language appears in the narration");
  }
  if (/(payable|remittance|tax due|vat payable|wht payable)/.test(narration)) {
    addHint("Tax payable", "the narration suggests a payable balance rather than an expense");
  }
  if (/(inventory|stock|purchase|restock|raw material)/.test(narration)) {
    addHint("Inventory", "inventory or stock language appears in the narration");
  }
  if (/(equipment|generator|laptop|vehicle|machine|asset purchase|furniture)/.test(narration)) {
    addHint(
      "Equipment and fixed assets",
      "the narration looks like a fixed-asset or equipment purchase"
    );
  }
