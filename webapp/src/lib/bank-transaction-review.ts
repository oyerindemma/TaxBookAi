...
import {
  buildEmptyWorkspaceBankTransactionWorkflowSummary,
  getWorkspaceBankTransactionWorkflowSummary,
  type WorkspaceBankTransactionWorkflowSummary,
} from "@/lib/bank-transaction-posting";
...
import {
  clearBankTransactionSnapshotLockData,
  enqueueRecalcForTransactionDates,
  markWorkspaceForRecalculationIfSnapshotLocked,
} from "@/lib/tax-snapshot-service";
import {
...
export type SerializedBankTransactionReviewDashboard = {
  ...
  workflow: Awaited<ReturnType<typeof getWorkspaceBankTransactionWorkflowSummary>>;
};

export type SerializedReviewQueuePreviewItem = {
  id: number;
...
export type SerializedReviewQueuePreview = {
  items: SerializedReviewQueuePreviewItem[];
  total: number;
  pendingCount: number;
...
function serializeLegacyBankTransactionMatch(
  ...
  if (match.bookkeepingDraft) {
    const bookkeepingBusinessName =
      match.bookkeepingDraft.upload.clientBusiness?.name ?? "Bookkeeping";
    ...
    return {
      ...
      target: {
        ...
        subtitle: `${bookkeepingBusinessName} · ${match.bookkeepingDraft.reviewStatus}`,
        ...
        clientBusinessName: bookkeepingBusinessName,
      },
    };
  }
...
function serializeLegacyBankTransaction(
  ...
  const bankAccountName = bankAccount?.name ?? "Unassigned account";
  ...
  return {
    ...
    bankAccount: {
      ...
      currency: bankAccount?.currency ?? transaction.currency,
    },
...
export function buildEmptyBankTransactionWorkflowSummary(
  workspaceId: number,
  input?: {
    requiresSetup?: boolean;
  }
...
type WorkflowSummarySetupSnapshot = {
...
async function getWorkspaceWorkflowSummarySetupSnapshot(
  workspaceId: number
): Promise<WorkflowSummarySetupSnapshot> {
  const [bankAccountCount, transactionCount, importCount, openReviewCount, clientBusinessCount] =
    await Promise.all([
      ...
      prisma.bankTransaction.count({
        where: {
          workspaceId,
          reviewStatus: {
            in: ["IMPORTED", "PENDING_REVIEW", "REVIEWED", "FLAGGED"],
          },
        },
      }),
...
function isValidEmptyWorkflowSummaryState(snapshot: WorkflowSummarySetupSnapshot) {
  if (snapshot.transactionCount === 0) {
    return true;
  }

  return (
    snapshot.bankAccountCount === 0 &&
    snapshot.importCount === 0 &&
    snapshot.openReviewCount === 0
  );
}

export function buildEmptyBankTransactionReviewDashboard(
  workspaceId: number
): SerializedBankTransactionReviewDashboard {
  return {
    accounts: [],
    clientBusinesses: [],
    transactions: [],
    summary: buildEmptyReviewSummary(),
    workflow: buildEmptyBankTransactionWorkflowSummary(workspaceId),
  };
}

export function buildEmptyReviewQueuePreview(input?: {
  requiresSetup?: boolean;
}): SerializedReviewQueuePreview {
  return {
    items: [],
    total: 0,
    pendingCount: 0,
...
const DASHBOARD_REVIEW_QUEUE_OPEN_STATUSES = [
  "IMPORTED",
  "PENDING_REVIEW",
  "REVIEWED",
  "FLAGGED",
...
export async function getWorkspaceDashboardReviewQueuePreview(
  workspaceId: number
): Promise<SerializedReviewQueuePreview> {
  if (!workspaceId || !Number.isInteger(workspaceId) || workspaceId <= 0) {
    return buildEmptyReviewQueuePreview();
  }

  const where: Prisma.BankTransactionWhereInput = {
    workspaceId,
    reviewStatus: {
      in: [...DASHBOARD_REVIEW_QUEUE_OPEN_STATUSES],
    },
  };

  const [
    bankAccountCount,
    totalWorkspaceTransactions,
    total,
    pendingCount,
    flaggedCount,
    readyToPostCount,
  ...
  if (total === 0) {
    return buildEmptyReviewQueuePreview({
      requiresSetup: bankAccountCount === 0 || totalWorkspaceTransactions === 0,
    });
  }
  ...
  return {
    items: items.map((item) => ({
      ...
      postingReadiness: item.postingReadiness ?? null,
      clientBusinessName: item.clientBusiness?.name ?? null,
      suggestedCategoryName: item.suggestedCategory?.name ?? item.suggestedCategoryName ?? null,
    })),
    total,
    pendingCount,
    flaggedCount,
...
export async function getWorkspaceDashboardReviewQueuePreviewSafe(
  workspaceId: number
): Promise<SerializedReviewQueuePreview> {
  try {
    return await getWorkspaceDashboardReviewQueuePreview(workspaceId);
  } catch (error) {
    console.error("[TaxBook:dashboard] Review queue preview failed", {
...
export async function getWorkspaceBankTransactionWorkflowSummarySafe(
  workspaceId: number
): Promise<WorkspaceBankTransactionWorkflowSummary> {
  const fallbackWorkspaceId = Number.isInteger(workspaceId) && workspaceId > 0 ? workspaceId : 0;
  const fallbackSummary = buildEmptyBankTransactionWorkflowSummary(fallbackWorkspaceId);
  ...
  try {
    ...
    return await getWorkspaceBankTransactionWorkflowSummary(workspaceId);
  } catch (error) {
    try {
      if (Number.isInteger(workspaceId) && workspaceId > 0) {
        const fallbackSnapshot = await getWorkspaceWorkflowSummarySetupSnapshot(workspaceId);
        if (isValidEmptyWorkflowSummaryState(fallbackSnapshot)) {
          return buildEmptyBankTransactionWorkflowSummary(workspaceId, {
            requiresSetup: fallbackSnapshot.requiresSetup,
          });
        }
      }
    } catch {
      // Keep the original error handling path for real failures.
    }

    logError(
      "bank-transaction-review",
      "Transaction workflow summary failed; returning an empty workflow summary.",
      error,
      {
        workspaceId,
      }
...
export async function getWorkspaceBankTransactionReviewData(
  ...
  await initializeWorkspaceIfEmpty(input.workspaceId);
  ...
  const [accounts, clientBusinesses, workflow] = await Promise.all([
    ...
    getWorkspaceBankTransactionWorkflowSummary(input.workspaceId),
  ]);
...
export async function getWorkspaceBankTransactionReviewDataSafe(
  input: WorkspaceBankTransactionReviewDataInput
) {
  try {
    return await getWorkspaceBankTransactionReviewData(input);
  } catch (error) {
    logError(
      "bank-transaction-review",
      "Transaction review loader failed; returning an empty review dashboard.",
      error,
      {
        workspaceId: input.workspaceId,
      }
    ...
    return buildEmptyBankTransactionReviewDashboard(input.workspaceId);
  }
}
...
export async function updateWorkspaceBankTransactionReview(input: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await markWorkspaceForRecalculationIfSnapshotLocked(tx, {
      workspaceId: input.workspaceId,
    ...
    await enqueueRecalcForTransactionDates(tx, {
      ...
      transactionDates: [existing.transactionDate, nextTransactionDate],
    ...
    if (nextReviewStatus === "REVIEWED" || nextReviewStatus === "POSTED") {
      await postBankTransactionToFinancialEngineWithExecutor(tx, {
        id: existing.id,
        workspaceId: input.workspaceId,
...
export async function bulkUpdateWorkspaceBankTransactionReviewStatus(input: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    for (const transaction of changedTransactions) {
      ...
      if (input.reviewStatus === "REVIEWED" || input.reviewStatus === "POSTED") {
        await postBankTransactionToFinancialEngineWithExecutor(tx, {
          id: transaction.id,
          workspaceId: input.workspaceId,
...
export async function deleteWorkspaceBankTransaction(input: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await enqueueRecalcForTransactionDates(tx, {
      workspaceId: input.workspaceId,
      userId: input.actorUserId,
      transactionDates: [existing.transactionDate],
      reason: "transaction_changed",
    });
