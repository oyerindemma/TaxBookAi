...
const BANKING_ENHANCED_TRANSACTION_QUERY_SUPPORT = {
  tables: ["BankTransaction"],
  columns: [
    "BankTransaction.source",
    "BankTransaction.reviewStatus",
    "BankTransaction.postingReadiness",
    "BankTransaction.accountingPostingStatus",
    "BankTransaction.normalizedDescription",
    "BankTransaction.normalizedMerchantName",
    "BankTransaction.autoBookkeepingConfidence",
    "BankTransaction.autoBookkeepingReason",
    "BankTransaction.autoBookkeepingProvider",
    "BankTransaction.autoBookkeepingProcessedAt",
...
const bankTransactionCompatibilitySelect = {
  id: true,
  transactionDate: true,
  description: true,
  reference: true,
...
type BankTransactionCompatibilityRecord = Prisma.BankTransactionGetPayload<{
  select: typeof bankTransactionCompatibilitySelect;
}>;
...
export type SerializedBankTransaction = {
  ...
  accountingPostingStatus: BankTransactionAccountingPostingStatus;
  possibleDuplicateOf: {
    id: number;
    transactionDate: string;
    description: string;
...
function buildBankingSummary(transactions: SerializedBankTransaction[]) {
  ...
  for (const transaction of transactions) {
    ...
    const isPosted =
      transaction.accountingPostingStatus === "POSTED" ||
    ...
    if (!isPosted && transaction.status === "UNMATCHED") {
      unmatched += 1;
    }
...
function filterSerializedTransactionsByQuery(
  transactions: SerializedBankTransaction[],
  query?: string | null
) {
  if (!normalizeString(query)) {
  ...
  return transactions.filter((transaction) => {
    const haystack = [
      transaction.description,
      transaction.reference ?? "",
      transaction.categorization.counterpartyName ?? "",
      transaction.categorization.suggestedCategoryName ?? "",
      transaction.bankAccount.name,
      transaction.clientBusiness?.name ?? "",
    ]
      .join(" ")
...
function serializeCompatibilityBankTransaction(
  transaction: BankTransactionCompatibilityRecord
): SerializedBankTransaction {
  const reviewStatus: BankTransactionReviewStatus = transaction.statementImport
    ? "IMPORTED"
    : "PENDING_REVIEW";
  const source: BankTransactionSource = transaction.statementImport ? "CSV_IMPORT" : "MANUAL";
  const postingReadiness: BankTransactionPostingReadiness = "NOT_READY";
  const resolvedTax = resolveBankTransactionTax({
    amountMinor: transaction.amount,
    description: transaction.description,
    reference: transaction.reference,
  ...
  return {
    ...
    taxTreatmentSource: resolvedTax.taxTreatmentSource,
    usesSuggestedTaxFallback: resolvedTax.usesSuggestedFallback,
    suggestionConfidence: transaction.confidenceScore ?? null,
    suggestionReason: null,
    normalizedDescription: null,
    normalizedMerchantName: null,
    autoBookkeepingConfidence: transaction.confidenceScore ?? null,
    ...
    matchedLedgerEntryId: transaction.matchedLedgerTransactionId ?? null,
    ...
    categorization: {
      ...
      provider: transaction.categorizationProvider,
      vatRelevance:
        transaction.suggestedVatTreatment === "NONE" ? "NOT_RELEVANT" : "RELEVANT",
      whtRelevance:
        transaction.suggestedWhtTreatment === "NONE" ? "NOT_RELEVANT" : "RELEVANT",
      vatRate: resolvedTax.vatRate,
      whtRate: resolvedTax.whtRate,
    },
    approvedMatch: null,
...
async function loadWorkspaceBankingTransactions(input: {
  ...
  try {
    ...
    const supportsEnhancedTransactionQuery = await hasPrismaDatabaseSupport(
      BANKING_ENHANCED_TRANSACTION_QUERY_SUPPORT
    );

    if (!supportsEnhancedTransactionQuery) {
      const transactions = await prisma.bankTransaction.findMany({
        where,
        orderBy: [{ transactionDate: "desc" }, { id: "desc" }],
        take: 120,
        select: bankTransactionCompatibilitySelect,
      });
      const serializedTransactions = filterSerializedTransactionsByQuery(
        transactions.map((transaction) => serializeCompatibilityBankTransaction(transaction)),
        input.query
      );
      ...
      return {
        ...
        summary: buildBankingSummary(serializedTransactions),
      };
    }
...
export async function importBankStatementCsv(input: {
  ...
  const clientBusinessId = await resolveClientBusinessIdOrDefault({
    workspaceId: input.workspaceId,
    clientBusinessId: input.clientBusinessId ?? account.clientBusinessId,
  });
...
export async function approveReconciliationMatch(input: {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    let approvedClientBusinessId: number | null = match.bankTransaction.clientBusinessId;
    ...
    if (match.matchType === "LEDGER_TRANSACTION") {
      ...
      approvedClientBusinessId =
        match.ledgerTransaction?.clientBusinessId ?? approvedClientBusinessId;

      if (!match.bankTransaction.clientBusinessId && match.ledgerTransaction?.clientBusinessId) {
        await markWorkspaceForRecalculationIfSnapshotLocked(tx, {
          workspaceId: input.workspaceId,
          transactionIds: [match.bankTransactionId],
        });
      ...
      approvedClientBusinessId = match.bookkeepingDraft.upload.clientBusinessId;
      ...
      const clientBusinessId = await resolveClientBusinessIdOrDefault({
        db: tx,
        workspaceId: input.workspaceId,
        clientBusinessId: match.bankTransaction.clientBusinessId ?? match.invoice.clientBusinessId,
      });
      approvedClientBusinessId = clientBusinessId;
    ...
    await tx.bankTransaction.update({
      ...
      data: {
        ...(approvedClientBusinessId ? { clientBusinessId: approvedClientBusinessId } : {}),
        status: "MATCHED",
...
export async function createManualLedgerMatch(input: {
  ...
  const result = await prisma.$transaction(async (tx) => {
    const workspaceId = await resolveWorkspaceIdOrDefault({
      db: tx,
      workspaceId: input.workspaceId,
    });
    ...
    const clientBusinessId = await resolveClientBusinessIdOrDefault({
      db: tx,
      workspaceId,
      clientBusinessId: input.payload.clientBusinessId ?? bankTransaction.clientBusinessId,
    });
...
export async function splitBankTransaction(input: {
  ...
  const transactionId = await prisma.$transaction(async (tx) => {
    ...
    const clientBusinessId = await resolveClientBusinessIdOrDefault({
      db: tx,
      workspaceId: input.workspaceId,
      clientBusinessId: input.clientBusinessId ?? bankTransaction.clientBusinessId,
    });
